...
async function cekKey(req, res, next) {
	...
	if (!apikey ) return res.json({ status : false, creator : `${creator}`, message : "Silahkan Masukan Apikey"})  

    let db = await User.findOne({apikey: apikey});
    if(db === null) {
		return res.json({ status : false, creator : `${creator}`, message : "Apikey Tidak Ditemukan"})  
		} else if(!db.isVerified) {
				return res.json({ status : false, creator : `${creator}`, message : "Silahkan Verifikasi Email Sebelum Menggunakan Apikey"})  
			} else if(db.limitApikey === 0) {
				return res.json({ status : false, creator : `${creator}`, message : "Apikey Sudah Mencapai Batas Limit"})  
			}else{
        return next();
    }
}
